				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("ITEM_ID") === oObject.ITEM_ID;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].ITEM_ID;
										} else {
											sCurrentId = aItemsNotInTheList[0].ITEM_ID;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ACMECatalog", {
											ITEM_ID : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("ITEM_ID"),
							title: oBindingContext.getProperty("ITEM")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "ACMECatalog (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'ACMECatalog (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("ITEM");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all ACMECatalog to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "ACMECatalog (" + iExpectedLength + ")"}),
									success : function () {
