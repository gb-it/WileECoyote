				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("EPISODE_ID") === oObject.EPISODE_ID;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].EPISODE_ID;
										} else {
											sCurrentId = aItemsNotInTheList[0].EPISODE_ID;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Episodes", {
											EPISODE_ID : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("EPISODE_ID"),
							title: oBindingContext.getProperty("EPISODE_NAME")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "Episodes (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'Episodes (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("EPISODE_NAME");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all Episodes to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "Episodes (" + iExpectedLength + ")"}),
									success : function () {
